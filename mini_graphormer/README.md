...
Adds α · 𝟙[i, j share a ring] as an additive attention bias, with rings
extracted via rdkit's smallest-set-of-smallest-rings (SSSR). Binary mask;
shows the steepest sensitivity to α of the three extensions. Same
four-config eval. Tables ordered by ascending α.
